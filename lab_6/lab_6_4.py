...
Примечание: В Python остаток от деления для отрицательных нечётных чисел также равен 1 (например, -3 % 2 == 1),
            поэтому проверка на нечётность через abs(x) % 2 == 1 эквивалентна x % 2 != 0.
"""
...
while i < len(mass):
    ...
    if abs(x) % 2 == 1:  # x — нечётный
        if cur_len == 0:
            ...
            prev = mass[i - 1]
            if abs(prev) % 2 == 1 and x * prev < 0:  # соседний элемент нечётный и знак чередуется
                cur_len += 1
            else:
                cur_len = 1
                cur_start = i
